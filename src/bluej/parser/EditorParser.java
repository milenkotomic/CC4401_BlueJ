		 ...
		 protected void beginVariableDecl(LocatableToken first)
	 {
		 ...
		 public ArrayList<MethodNode>manyParameters(int threshold){
			 ...
			 for(MethodNode mn:methods){
				 if(mn.getParamNames().size()> threshold){
					 result.add(mn);
				 }
			 }
			 return result;
		 ...
		 public void parseStmtBlock()
		 {
			 ...
			 while(true) {
				 ...
				 if (ntoken != null) {
					 ...
					 if (ntoken == token) {
						 nextToken();
						 error("Invalid beginning of statement.", token.getLine(), token.getColumn(), token.getEndLine(), token.getEndColumn());
						 continue;
					 }
				 }
			 }
